    ...
    return (
        <section className="max-w-6xl mx-auto px-4 py-16">
            ...
            <div className="grid md:grid-cols-3 gap-8">
                {/* Strong */}
                <div className="bg-nature-900/50 rounded-xl p-6 border border-garnet-800/50 relative overflow-hidden group">
                    <div className="absolute top-0 left-0 w-1 h-full bg-garnet-500/60" />
                    <h3 className="text-xl font-bold mb-6 flex items-center gap-2 text-white">
                        <CheckCircle2 className="w-5 h-5 text-garnet-400" />
                        Superpowers
                    </h3>
                    <ul className="space-y-4">
                        {PROFILE.skills.strong.map((skill, i) => (
                            <li key={i} className="flex items-start gap-3 text-cream-200">
                                <CheckCircle2 className="w-5 h-5 text-garnet-400/60 flex-shrink-0 mt-0.5" />
                                <span>{skill}</span>
                            </li>
                        ))}
                    </ul>
                ...
                <div className="bg-nature-900/50 rounded-xl p-6 border border-amber-900/30 relative overflow-hidden">
                    ...
                    <ul className="space-y-4">
                        {PROFILE.skills.gaps.map((skill, i) => (
                            <li key={i} className="flex items-start gap-3 text-cream-200">
                                <AlertCircle className="w-5 h-5 text-amber-400/60 flex-shrink-0 mt-0.5" />
                                <span>{skill}</span>
                            </li>
                        ))}
                    </ul>
